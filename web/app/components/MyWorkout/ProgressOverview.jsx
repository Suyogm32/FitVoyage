...
const CompletionRing = ({ rate }) => {
  ...
  return (
    <svg width="128" height="128" viewBox="0 0 128 128" role="img" aria-label={`${pct}% completion rate`}>
      <circle cx="64" cy="64" r={RADIUS} fill="none" stroke="hsl(var(--muted))" strokeWidth="13" />
      <circle
        cx="64"
        cy="64"
        r={RADIUS}
      ...
      />
      <text x="64" y="62" textAnchor="middle" style={{ fontSize: 26, fontWeight: 500, fill: "currentColor" }}>
        {rate === null ? "—" : `${pct}%`}
      </text>
      <text x="64" y="82" textAnchor="middle" style={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }}>
        completed
      </text>
    </svg>
  );
...
const ProgressOverview = ({ stats, range, onRangeChange }) => {
  ...
  const rangeLabel = RANGES.find((r) => r.key === range)?.label.toLowerCase() || "month";
            ...
            <div className="flex flex-col gap-3">
              {muscleGroups.map((group) => (
                <div key={group.bodyPart} className="flex items-center gap-3">
                  ...
                  <Typography variant="body2" className="w-10 text-right shrink-0">
                    {group.sets}
                  </Typography>
                </div>
              ))}
          ...
          {stats.topExercises.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {stats.topExercises.map((ex) => (
                ...
                >
                  <Typography textTransform="capitalize">{ex.exerciseName}</Typography>
...
const StatCard = ({ label, value, unit }) => (
  <div className={`${cardClass} p-5`}>
    ...
    <div className="flex items-baseline gap-1 mt-1">
      <span className="text-4xl font-bold leading-none" style={{ color: "hsl(var(--primary))" }}>
        {value}
      </span>
      {unit && (
