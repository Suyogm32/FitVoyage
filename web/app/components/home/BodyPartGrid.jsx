...
const BodyPartGrid = () => {
  ...
  // Hide the section only once we know there's nothing to show. Returning
  // null while still loading made the whole browse section vanish during a
  // cold start.
      ...
      <div className="flex items-end justify-between gap-4 mb-7 flex-wrap">
        ...
        {loading
          ? Array.from({ length: 10 }).map((_, index) => (
              <div
                key={index}
                className="rounded-2xl border border-border bg-card animate-pulse h-[190px]"
              />
            ))
