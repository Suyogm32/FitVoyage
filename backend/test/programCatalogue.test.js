...
test("caps how many exercises one equipment type contributes", () => {
  ...
  const selected = selectCatalogue(chest, { maxPerEquipment: 5 });
  assert.equal(selected.length, 5);
});

test("keeps variety across equipment within a body part", () => {
  const chest = [
    ...Array.from({ length: 10 }, (_, i) => exercise(`d${i}`, `db press ${i}`, "chest", "dumbbell")),
    ...Array.from({ length: 10 }, (_, i) => exercise(`b${i}`, `bb press ${i}`, "chest", "barbell")),
  ];
  const selected = selectCatalogue(chest, { maxPerEquipment: 3 });
...
test("prefers shorter names as a proxy for staple movements", () => {
  const chest = [
    exercise("a", "barbell bench press with chains on a smith machine", "chest", "barbell"),
    exercise("b", "barbell bench press", "chest", "barbell"),
  ];
  const selected = selectCatalogue(chest, { maxPerEquipment: 1 });
...
test("rejects reps arrays that don't match the set count", () => {
  const plan = {
    days: [{ day: "mon", exercises: [{ exerciseId: "real", sets: 3, reps: [10, 10] }] }],
  };
  const { days, dropped } = validatePlan(plan, ["real"]);
...
test("rejects invalid day keys", () => {
  const plan = {
    days: [{ day: "someday", exercises: [{ exerciseId: "real", sets: 1, reps: [10] }] }],
  };
...
test("a day left with no valid exercises is reported, not silently kept", () => {
  const plan = {
    days: [{ day: "mon", exercises: [{ exerciseId: "nope", sets: 1, reps: [10] }] }],
  };
  const { days, dropped } = validatePlan(plan, ["real"]);
  assert.equal(days.length, 0);
...
test("skips trimming when the pool is already small", () => {
  ...
  const small = Array.from({ length: 23 }, (_, i) =>
    exercise(`e${i}`, `ez bar movement ${i}`, i % 2 ? "back" : "upper arms", "ez barbell"),
  );
  assert.equal(selectCatalogue(small).length, 23);
});

test("bodyweight is always available, without duplicating it", () => {
  assert.deepEqual(withBodyweight(["ez barbell"]), ["ez barbell", "body weight"]);
...
test("excludes cardio for muscle and strength goals, keeps it otherwise", () => {
  const pool = [
    ...Array.from({ length: 15 }, (_, i) => exercise(`c${i}`, `cardio ${i}`, "cardio", "body weight")),
    ...Array.from({ length: 15 }, (_, i) => exercise(`b${i}`, `back ${i}`, "back", "body weight")),
  ];
...
test("keeps cardio rather than leaving too small a pool", () => {
  ...
  const pool = [
    ...Array.from({ length: 12 }, (_, i) => exercise(`c${i}`, `cardio ${i}`, "cardio", "stationary bike")),
